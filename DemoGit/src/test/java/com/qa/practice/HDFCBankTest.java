...
public class HDFCBankTest {
	...
	public void test1() throws InterruptedException {
		...
		//		WebElement popBanner = driver.findElement(By.xpath("//img[contains(@class,'popupbanner at-element-click-tracking')]"));
		...
		try {
			...
			//		driver.findElement(By.xpath("//img[@class='popupbanner at-element-click-tracking']/../following-sibling::img[@class='popupCloseButton']")).click();
		...
		if (allwindows.size()>1) {
			...
			while(itr.hasNext())
			{
				...
				if(firstwindow!=secondwindow) {
					...
					WebElement contNetBanking = driver.findElement(By.xpath("(//a[text()='Continue to NetBanking'])[2]"));
		...
		for(String allinsidewindowshandles : driver.getWindowHandles()) {
			...
			if (driver.switchTo().window(allinsidewindowshandles).getTitle().equals("Welcome to HDFC Bank NetBanking"))
			{
				...
				//								System.out.println(driver.findElement(By.xpath("//td[contains(text(),'User ID / Customer ID')]")).getText());
				...
				//								WebElement lblwrongpass = driver.findElement(By.xpath("//strong[contains(text(),'The Customer ID/IPIN (password) you have entered is invalid, please try again')]"));
				//								wait.until(ExpectedConditions.visibilityOf(lblwrongpass)).isDisplayed();
